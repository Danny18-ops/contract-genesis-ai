export const generateDetailedContract = (data: any) => {
  const contractTemplates = {
    ...
    jobOffer: generateJobOfferContract,
    business: generateBusinessContract,
    carRental: generateCarRentalContract,
    storage: generateStorageContract
  };
...
const generateOrganizationHeader = (orgData: any) => {
  if (!orgData || !orgData.name) return '';
...
const generateFooter = (footerText: string, pageNumber: number = 1) => {
  return `
───────────────────────────────────────────────────────────────────────────────
${footerText || 'Generated by GenContract AI'} | Page ${pageNumber} | ${getCurrentDate()}
───────────────────────────────────────────────────────────────────────────────
`;
};
...
const generateGenericContract = (data: any) => {
  return `
${generateOrganizationHeader(data.organizationData)}

                                GENERAL SERVICE AGREEMENT

This Agreement is entered into on ${getCurrentDate()} between:

PARTY 1: ${data.organizationData?.name || '[ORGANIZATION]'}
PARTY 2: ${data.party2 || '[SECOND PARTY]'}
...
_____________________________     _____________________________
${data.organizationData?.name || '[PARTY 1]'}    ${data.party2 || '[PARTY 2]'}

Date: _______________________     Date: _______________________

${generateFooter(data.footerText)}
  `.trim();
};

const generateJobOfferContract = (data: any) => {
  return `
${generateOrganizationHeader(data.organizationData)}
...
Dear ${data.candidate || '[CANDIDATE NAME]'},

We are pleased to extend this formal offer of employment for the position of ${data.position || '[JOB POSITION]'} with ${data.organizationData?.name || '[COMPANY NAME]'}.

POSITION DETAILS:
• Job Title: ${data.position || '[JOB POSITION]'}
• Department: ${data.department || '[DEPARTMENT]'}
• Reporting Manager: ${data.reportingManager || '[MANAGER NAME]'}
• Employment Start Date: ${data.startDate || '[START DATE]'}
• Employment Type: Full-time, Regular

COMPENSATION AND BENEFITS:
• Annual Salary: $${data.salary || '[SALARY AMOUNT]'}
• Payment Frequency: Bi-weekly
• Benefits Package: ${data.benefits || 'Health insurance, dental, vision, 401(k) matching, paid time off'}

TERMS AND CONDITIONS:
1. This offer is contingent upon successful completion of background check and reference verification.
2. Employment is at-will and may be terminated by either party with appropriate notice.
3. You will be required to sign our standard confidentiality and non-disclosure agreement.
4. This position requires adherence to all company policies and procedures.

Please confirm your acceptance of this offer by signing below and returning this letter by [DATE].
...
${data.organizationData?.name || '[HIRING MANAGER]'}
Hiring Manager
...
const generateBusinessContract = (data: any) => {
  return `
${generateOrganizationHeader(data.organizationData)}

                                BUSINESS AGREEMENT

This Business Agreement is entered into on ${getCurrentDate()} between:

PARTY 1: ${data.organizationData?.name || '[ORGANIZATION]'}
Address: ${data.organizationData?.address || '[ADDRESS]'}

PARTY 2: ${data.party2 || '[SECOND PARTY]'}
Address: [PARTY 2 ADDRESS]

PURPOSE:
${data.businessPurpose || 'This agreement establishes the terms for a business relationship between the parties.'}

AGREEMENT VALUE:
Total Value: $${data.agreementValue || '[AMOUNT]'}

KEY DELIVERABLES:
${data.deliverables || '• [Deliverable 1]\n• [Deliverable 2]\n• [Deliverable 3]'}

TIMELINE:
Project Duration: ${data.timeline || '[TIMELINE]'}

TERMS AND CONDITIONS:
1. Both parties agree to fulfill their obligations as outlined in this agreement.
2. Payment terms: [PAYMENT TERMS]
3. Either party may terminate with 30 days written notice.
4. This agreement is governed by applicable business laws.

SIGNATURES:

_________________________    Date: ___________
${data.organizationData?.name || '[PARTY 1]'}
...
const generateCarRentalContract = (data: any) => {
  return `
${generateOrganizationHeader(data.organizationData)}

                              CAR RENTAL AGREEMENT

Rental Agreement Number: [AGREEMENT NUMBER]
Date: ${getCurrentDate()}

RENTAL COMPANY: ${data.organizationData?.name || '[RENTAL COMPANY]'}
Address: ${data.organizationData?.address || '[COMPANY ADDRESS]'}

RENTER INFORMATION:
Name: ${data.renter || '[RENTER NAME]'}
Driver's License: ${data.driverLicense || '[LICENSE NUMBER]'}
Contact: [RENTER CONTACT]

VEHICLE INFORMATION:
Vehicle: ${data.vehicleDetails || '[MAKE/MODEL/YEAR]'}
License Plate: [PLATE NUMBER]
VIN: [VEHICLE IDENTIFICATION NUMBER]

RENTAL DETAILS:
Rental Period: ${data.rentalPeriod || '[RENTAL PERIOD]'}
Daily Rate: $${data.dailyRate || '[DAILY RATE]'}
Mileage Limit: ${data.mileageLimit || '[MILEAGE LIMIT]'} miles
Pickup Location: ${data.pickupLocation || '[PICKUP LOCATION]'}
Return Location: [RETURN LOCATION]

TERMS AND CONDITIONS:
1. Renter must be at least 21 years old with valid driver's license.
2. Vehicle must be returned with same fuel level as pickup.
3. Renter is responsible for all traffic violations and parking tickets.
4. Insurance coverage as per rental company policy.
5. Additional charges apply for excess mileage, late return, or damage.

SIGNATURES:

_________________________    Date: ___________
${data.organizationData?.name || '[RENTAL COMPANY]'}
Authorized Representative

_________________________    Date: ___________
${data.renter || '[RENTER NAME]'}
Renter
...
const generateStorageContract = (data: any) => {
  return `
${generateOrganizationHeader(data.organizationData)}

                           STORAGE CONTAINER LEASE AGREEMENT

Lease Agreement Number: [AGREEMENT NUMBER]
Date: ${getCurrentDate()}

STORAGE FACILITY: ${data.organizationData?.name || '[STORAGE FACILITY]'}
Address: ${data.storageLocation || data.organizationData?.address || '[FACILITY ADDRESS]'}

TENANT INFORMATION:
Name: ${data.tenant || '[TENANT NAME]'}
Address: [TENANT ADDRESS]
Contact: [TENANT CONTACT]

STORAGE UNIT DETAILS:
Unit Number: ${data.unitNumber || '[UNIT NUMBER]'}
Unit Size: ${data.unitSize || '[UNIT DIMENSIONS]'}
Monthly Fee: $${data.monthlyFee || '[MONTHLY FEE]'}
Access Hours: ${data.accessHours || '24/7 access with key code'}

LEASE TERMS:
Rental Duration: ${data.rentalDuration || '[RENTAL PERIOD]'}
Security Deposit: $[DEPOSIT AMOUNT]
Payment Due: 1st of each month

TERMS AND CONDITIONS:
1. Tenant responsible for securing unit with own lock.
2. No hazardous, illegal, or perishable items allowed.
3. Facility not responsible for theft, damage, or loss of stored items.
4. 30-day notice required for lease termination.
5. Late fees apply after 5-day grace period.

ACCESS AND SECURITY:
• Access code: [ACCESS CODE]
• Gate hours: [GATE HOURS]
• Security features: [SECURITY FEATURES]

SIGNATURES:

_________________________    Date: ___________
${data.organizationData?.name || '[STORAGE FACILITY]'}
Facility Manager
