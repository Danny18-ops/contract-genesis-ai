export const generateDetailedContract = (data: any): string => {
  console.log('Generating detailed contract with data:', data);
  ...
  const {
    ...
    dynamicFields,
    language = 'en',
    footerText = 'Generated by GenContract AI'
  } = data;
  ...
  if (contractType === 'professionalRental') {
    return generateProfessionalRentalContract(data);
  }
...
const generateFlexibleContract = (data: any): string => {
  ...
  let contract = `
${dynamicFields.contractTitle || 'Custom Agreement'}

This agreement is entered into on ${currentDate} between:

PARTY 1: ${organizationData.name}
Email: ${organizationData.email || 'Not provided'}

PARTY 2: ${dynamicFields.party2}
Email: ${dynamicFields.party2Email || 'Not provided'}

PURPOSE:
${dynamicFields.contractPurpose}

`;

  // Add custom sections
  if (dynamicFields.customSections && dynamicFields.customSections.length > 0) {
    contract += `TERMS AND CONDITIONS:\n\n`;
    
    dynamicFields.customSections.forEach((section: any, index: number) => {
      if (section.content && section.content.trim()) {
        contract += `${index + 1}. ${section.emoji} ${section.title.toUpperCase()}\n`;
  ...
  if (dynamicFields.includeTable && dynamicFields.tableRows && dynamicFields.tableRows.length > 0) {
    contract += `${dynamicFields.tableTitle || 'ITEMIZED DETAILS'}:\n\n`;
    
    // Table header
    contract += `${'Item'.padEnd(20)} | ${'Description'.padEnd(25)} | ${'Amount/Value'.padEnd(15)} | Date\n`;
    contract += `${'-'.repeat(20)} | ${'-'.repeat(25)} | ${'-'.repeat(15)} | ${'-'.repeat(12)}\n`;
    
    // Table rows
    dynamicFields.tableRows.forEach((row: any) => {
      if (row.item && row.item.trim()) {
        contract += `${(row.item || '').padEnd(20)} | ${(row.description || '').padEnd(25)} | ${(row.amount || '').padEnd(15)} | ${row.date || ''}\n`;
      }
    });
    contract += `\n`;
  }
  ...
  if (dynamicFields.enableSignatures) {
    contract += `SIGNATURES:\n\n`;
    contract += `Party 1 Signature: ${dynamicFields.yourSignature || '________________________'}\n`;
    contract += `Date: ${currentDate}\n\n`;
    contract += `Party 2 Signature: ${dynamicFields.otherPartySignature || '________________________'}\n`;
  ...
  // Add footer
  contract += `${data.footerText || 'Created with GenContract — Make It Official! 📝'}`;

  return contract.trim();
};

const generateProfessionalRentalContract = (data: any): string => {
  const startDate = new Date(data.leaseStartDate).toLocaleDateString();
  const endDate = new Date(data.leaseEndDate).toLocaleDateString();
  
  return `
RESIDENTIAL LEASE AGREEMENT

This Lease Agreement ("Agreement") is entered into on ${new Date().toLocaleDateString()}, between ${data.landlordName} ("Landlord") and ${data.tenantName} ("Tenant").

PROPERTY INFORMATION
Property Address: ${data.propertyAddress}${data.unitNumber ? ', Unit ' + data.unitNumber : ''}

LEASE TERM
Lease Period: ${startDate} to ${endDate} (${data.leaseType === 'fixed' ? 'Fixed Term' : 'Month-to-Month'})
Monthly Rent: $${data.monthlyRent}
Rent Due Date: ${data.rentDueDate} of each month
Security Deposit: $${data.securityDeposit}
${data.lateFeeAmount ? `Late Fee: $${data.lateFeeAmount} for payments received after the 5th of the month` : ''}
...
- Internet: Paid by ${data.internetPaidBy}

ADDITIONAL CHARGES
${data.parkingCharges ? `Parking: $${data.parkingCharges} per month` : ''}
${data.storageCharges ? `Storage: $${data.storageCharges} per month` : ''}

LEASE TERMS AND CONDITIONS
1. USE OF PREMISES: The premises shall be used exclusively as a private residential dwelling.

2. PETS: ${data.petPolicy ? 'Pets are allowed subject to additional terms and pet deposit.' : 'No pets are allowed on the premises without prior written consent.'}

3. MAINTENANCE: ${data.maintenanceResponsibilities || 'Landlord is responsible for major repairs. Tenant is responsible for routine maintenance and minor repairs under $100.'}

4. TERMINATION: ${data.moveOutNotice} days written notice is required for termination of this lease.

5. GOVERNING LAW: This agreement shall be governed by the laws of ${data.governingLaw || 'the state where the property is located'}.

6. SECURITY DEPOSIT: The security deposit will be held according to state law and returned within the legally required timeframe after lease termination, minus any lawful deductions.

7. RENT PAYMENT: Rent is due on the ${data.rentDueDate} of each month. Payment should be made to the landlord at the address provided above unless otherwise specified.

8. PROPERTY CONDITION: Tenant acknowledges receiving the premises in good condition and agrees to return it in the same condition, normal wear and tear excepted.

9. MODIFICATIONS: Any modifications to this agreement must be in writing and signed by both parties.

10. ENTIRE AGREEMENT: This agreement constitutes the entire agreement between the parties and supersedes all prior negotiations, representations, or agreements.
...
State of ${data.governingLaw || '_________'}
County of ___________

On this _____ day of _______, 20__, before me personally appeared ${data.landlordName} and ${data.tenantName}, who proved to me on the basis of satisfactory evidence to be the persons whose names are subscribed to the within instrument and acknowledged to me that they executed the same in their authorized capacities.

Notary Public: _______________________

Generated by GenContract — Professional Series
  `;
};
